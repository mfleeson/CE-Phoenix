  ...
  class category_tree {
    ...
    public function __construct() {
      ...
      if ( isset($_category_tree_data) ) {
       ...
       //ORIG $categories_query = tep_db_query("select c.categories_id, c.parent_id, c.categories_image, cd.categories_name, cd.categories_description, cd.categories_seo_description, cd.categories_seo_keywords, cd.categories_seo_title from categories c, categories_description cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id. "' order by c.parent_id, c.sort_order, cd.categories_name");
		  ...
		  while ($catman = tep_db_fetch_array($categories_management_structure_query)) {
		   ...
		   $categories_query = tep_db_query("select c.categories_id, c.parent_id, c.categories_image, cd.categories_name, cd.categories_description, cd.categories_seo_description, cd.categories_seo_keywords, cd.categories_seo_title,cm.* from categories c, categories_description cd, categories_management cm where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id. "' and cm.categories_id = c.categories_id and cm.active=1 order by c.parent_id, c.sort_order, cd.categories_name");
		  
        while ( $categories = tep_db_fetch_array($categories_query) ) {
			$arkeys = array_keys($categories);
          $this->_data[$categories['parent_id']][$categories['categories_id']] = 
			...
			{
				 $this->_data[$categories['parent_id']][$categories['categories_id']][$catman] =$categories[$catman];
			}
        }
